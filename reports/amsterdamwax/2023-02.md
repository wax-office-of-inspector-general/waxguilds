...
Last month we created and released a Hyperion health checker for haproxy which monitors the last indexed block and number of unindexed blocks, and stops sending requests to it if they exceed the thresholds. This one is still running smoothly. More information can be found here. - https://github.com/cc32d9/eosio-haproxy
...
### *NEW* HTTP API Memento
Memento is our lightweight history service for Antelope blockchains which has been greatly received by the community. To continue supporting Memento we have no released a HTTP API on top of the Memento database. It supports both MySQL and Postgres backends. Additionally, it also has a GraphQL interface in addition to the RESTful interface. More information can be found here: https://github.com/Antelope-Memento/antelope_memento_api and https://github.com/Antelope-Memento/antelope_memento/blob/main/MEMENTO_PUBLIC_ACCESS.md

### *NEW* Chronicle release 2.3
We have always maintained and updated the Chronicle project, which aims to build a scalable and reliable toolset for maintaining Antelope blockchain history in a database. We have now released the newest version which contains bug fixes. More information can be found here: https://github.com/EOSChronicleProject/eos-chronicle/releases/tag/v2.3

### *UPDATED* NFT-releases.com
Since partnering up with NFT-releases.com to create a central hub for NFT collectors to stay up-to-date on the latest NFT news and collections we have continued assisting them with their strategy, marketing, and technology. A new interface for the website will be launched around the time this report will be reviewed, with many more exciting updates coming in the near future. A first version of the website can be found at https://nft-releases.com/.

### *UPDATED* Antelope/Mandel/WAX-leap
We have now shipped the latest updates (major fixes in stability). In main branch, tag v3.1.4wax01. In wax-leap-3.2 branch, tag v3.2.1wax01. And in wax-leap-head branch, synched with the main branch of Leap (future 4.0 release). More information can be found here: https://github.com/cc32d9/wax-leap

Additionally, we have done bug reporting and testing for WAX. More information on the reported issues can be found here: https://github.com/AntelopeIO/leap/issues/669
...
EOSIO Light API: 355k requests per hour on average, with 98% of the requests coming from WAX
