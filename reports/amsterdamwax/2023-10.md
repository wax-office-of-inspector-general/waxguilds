...
We recently discovered a small issue with the latest Leap release, which we outlined here: https://github.com/AntelopeIO/leap/issues/1694.

Additionally, we are continuously standing by for new releases.
...
WAX RPC: 293k API requests per hour on average
EOSIO Light API: 103k requests per hour on average, with 96% of the requests coming from WAX
Atomic Assets API: 25k requests per hour on average
WAX Hyperion: 35k history requests per hour on average
